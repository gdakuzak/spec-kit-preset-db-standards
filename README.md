...
```bash
specify preset add --from https://github.com/gdakuzak/spec-kit-preset-db-standards/archive/refs/tags/v0.8.2.zip
```
...
- No database at all.
- A NoSQL store with no reference file yet (see `references/`) — the generic
  templates assume a relational engine; DynamoDB and MongoDB are the covered
  exceptions.
...
Found a gap, a wrong default, or want another engine covered? [Open an
issue](https://github.com/gdakuzak/spec-kit-preset-db-standards/issues).
